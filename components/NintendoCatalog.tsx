...
import {
  NINTENDO_REGIONS,
  nintendoAmountLabel,
} from "@/lib/products/nintendo-catalog";
import type { NintendoProduct } from "@/lib/products/nintendo-catalog-quote";
import { useStore } from "@/store/useStore";

const REGION_ORDER = ["US", "EU", "UK", "PL"];
const REGION_META = new Map(NINTENDO_REGIONS.map((r) => [r.region, r]));
// "UK" is our region code; the flag set files it under its ISO code, GB.
const FLAG: Record<string, ComponentType<{ className?: string }>> = { US, EU, UK: GB, PL };
...
export default function NintendoCatalog({ products }: { products: NintendoProduct[] }) {
  ...
  function cartTitle(p: NintendoProduct) {
    const meta = REGION_META.get(p.region);
    return `Nintendo eShop ${nintendoAmountLabel(p.amountMajor, p.currency)} (${meta?.label ?? p.region})`;
  }

  function buy(p: NintendoProduct, goToCart: boolean) {
    addToCart({
  ...
  return (
    <div>
      {/* Header + generic Nintendo card (placeholder, no denomination/country) */}
      <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-10">
        <div>
          <p className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.2em] text-[var(--text-muted)]">
            <BsNintendoSwitch className="h-4 w-4 text-[#e60012]" />
            Nintendo
          </p>
          ...
          <p className="mt-4 max-w-sm text-base leading-7 text-[var(--text-muted)]">
            Пополните баланс Nintendo Account для игр, дополнений и подписки
            Nintendo Switch Online. Выберите регион и сумму.
          </p>
        </div>
        ...
        <div className="relative hidden items-center justify-center lg:flex">
          <div
            aria-hidden="true"
            className="pointer-events-none absolute left-1/2 top-1/2 h-[280px] w-[280px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#e60012] opacity-15 blur-[90px]"
          />
          <div
        ...
        <div className="mt-3 flex flex-wrap gap-2.5">
          {amounts.map((p) => {
            ...
            return (
              ...
              >
                {nintendoAmountLabel(p.amountMajor, p.currency)}
              </button>
            );
          })}
        </div>
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-[family-name:var(--font-unbounded)] text-lg font-bold leading-snug text-[var(--ink)]">
                Nintendo eShop
                <br />
                {nintendoAmountLabel(selected.amountMajor, selected.currency)} ·{" "}
